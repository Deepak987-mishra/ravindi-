...
import { View, Text, useWindowDimensions, TextInput, Image, ScrollView, StyleSheet, TouchableOpacity, Animated, FlatList } from 'react-native';
...
const GroupClass = () => {
    ...
    const validationSchema = Yup.object().shape({
        price: Yup.number().positive().integer().required().label("Enter Price"),
    ...
    const { createClassTabIndex, classTypeList, timeZoneList, setAvailabilityDate, setAvailabilityTime } = useSelector(state => ({
        ...
        setAvailabilityDate: state.createClassReducer.setAvailabilityDate,
        setAvailabilityTime: state.createClassReducer.setAvailabilityTime,
    }));



    //state variable for timezone(Ist,Gmt)
    const [selectedTime, setSelectedTime] = useState();
    //state variable for total studensts allowed
    ...
    const handleCreateclass = (values) => {
        ...
        let body = {
            ...
            "image": image,
            "total_students_allowed": count,
            "start_time": setAvailabilityTime.startTime,
            "end_time": setAvailabilityTime.endTime,
            "availability": setAvailabilityDate,
        }
        if (!(body.class_id || body.timezone || body.total_students_allowed)) {
        ...
        if (!body.total_students_allowed) {
            Alert.alert("Select Number of studenst ")
            return
        }


        dispatch({
            type: types.CREATE_GROUP_CLASS,
            payload: body
        })
                        ...
                        >
                            {({ handleChange, handleSubmit, errors, setFieldTouched, setFieldValue, touched, values }) => (
                                <>
                                    ...
                                    <TouchableOpacity onPress={() => navigation.navigate('SetAvailability')} style={{
                                        flexDirection: 'row',
                                        alignItems: "center",
                                        justifyContent: 'space-between', marginTop: hpx(12),
                                        width: wpx(300)
                                    }}>
                                        <Text style={styles.totalStudent}>Set Availability</Text>

                                        <Image source={Icons.rightArrowBlack} style={{
                                            height: hpx(14),
                                            width: wpx(14),
                                            resizeMode: 'contain',
                                        }} />
                                    ...
                                    <View style={{
                                        flexDirection: 'row', marginTop: hpx(35), width: wpx(310), justifyContent: "space-between",
                                        alignItems: "center"
                                    }}>
                                        <Text style={styles.totalStudent}>Total Students Allowed*</Text>
                                        ...
                                        <View style={styles.studentContainer}>
                                            ...
                                            <Text style={styles.counterText}
                                                onChangeText={handleChange("count")}>{count}</Text>
                                    ...
                                    {/* <View style={styles.selectClasscontainer}>
                                        <Text style={styles.selectClasstext}>Select Class Type</Text>
                                        ...
                                        <View style={styles.classContainer}>
                                            <Picker
                                                ...
                                                {classTypeList.map((item, index) => <Picker.Item label={item.title} value={item.id.toString()} />)}
                                    ...
                                    <View style={styles.timezoneView}>
                                        ...
                                        <View style={styles.timezonePickerView}>
                                            <Picker
                                                style={styles.picker}
                                                ...
                                                <Picker.Item label="IST" value="IST + 5:30" />
                                                <Picker.Item label="GMT" value="GMT + 12:30 " />
                                            </Picker>
                                        </View>
                                    </View> */}
                                    <View style={styles.selectClasscontainer}>
...
const styles = StyleSheet.create({
    ...
    tabItem: {
        ...
        padding: wp(3),
        flexDirection: 'row',
        width: wpx(140), height: hpx(35), marginRight: wpx(32)
    ...
    buttonView: {
        marginTop: hpx(44), alignSelf: "center"
    },
    uploadContainer: {
        flexDirection: 'column',
        marginTop: hpx(26),
